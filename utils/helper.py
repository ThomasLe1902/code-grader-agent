...
def cleanup_repository(repo_path: str) -> bool:
    ...
    Args:
        repo_path (str): Path to the repository directory to delete
...
def cleanup_all_repositories() -> int:
    """Clean up all repositories in the repo folder.

    Returns:
        int: Number of repositories successfully cleaned up
    """
    if not os.path.exists(REPO_FOLDER):
        logger.info("No repo folder exists, nothing to cleanup")
        return 0
    ...
    try:
        for item in os.listdir(REPO_FOLDER):
            ...
            if os.path.isdir(item_path):
                if cleanup_repository(item_path):
                    cleaned_count += 1

        # Remove the repo folder itself if it's empty
        if cleaned_count > 0 and not os.listdir(REPO_FOLDER):
            os.rmdir(REPO_FOLDER)
            logger.info(f"Removed empty repo folder: {REPO_FOLDER}")
